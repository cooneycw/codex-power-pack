...
- [Define native Codex session identity and claims](tickets/02-define-session-identity.md): use the native Codex thread UUID as durable session identity, fence every worker and coordinator process generation, and bind assignments and authoritative transitions to durable policy and provenance evidence. The normative contract is [`docs/native-codex-wave-contract.md`](../../native-codex-wave-contract.md).

## Not yet specified

- The exact CPP Flow source snapshot, helper set, and CxPP runtime overlays needed
  by the chosen milestone.
- The detailed Spec Kit input and PR-watch contracts. Gate, assignment, identity,
  generation, and recovery semantics are settled by the native wave contract.
