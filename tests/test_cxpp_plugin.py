"""Behavioral guardrails for the CxPP host-setup fallback."""

from __future__ import annotations

import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SKILLS = ROOT / ".codex" / "skills"
PLUGIN = ROOT / "plugins" / "cxpp"
MARKETPLACE = ROOT / ".agents" / "plugins" / "marketplace.json"
HOOK_TRANSITION = ROOT / "scripts" / "cxpp-hook-transition.py"

FULL_FAMILIES = (
    "project",
    "spec",
    "flow",
    "github",
    "cicd",
    "secrets",
    "woodpecker",
    "security",
    "agents-md",
    "documentation",
    "qa",
    "evaluate",
    "second-opinion",
    "self-improvement",
    "cxpp",
    "claude",
)

RECOMMENDED_FAMILIES = tuple(
    family for family in FULL_FAMILIES if family not in {"woodpecker", "evaluate", "second-opinion"}
)


def read_skill(name: str) -> str:
    return (SKILLS / name / "SKILL.md").read_text(encoding="utf-8")


def test_cxpp_skills_are_native_and_packaged() -> None:
    for name in ("cxpp-init", "cxpp-update", "cxpp-status"):
        source = SKILLS / name / "SKILL.md"
        packaged = PLUGIN / "skills" / name / "SKILL.md"

        assert source.is_file()
        assert packaged.read_bytes() == source.read_bytes()
        assert "<!-- GENERATED by claude-power-pack" not in source.read_text(encoding="utf-8")

    assert (PLUGIN / "scripts/cxpp-hook-transition.py").read_bytes() == HOOK_TRANSITION.read_bytes()


def test_cxpp_init_requires_explicit_consent_and_preserves_boundaries() -> None:
    text = read_skill("cxpp-init")

    for needle in [
        "explicit approval",
        "Never request, print, read, or persist secret values.",
        "Never start, stop, deploy, update, or otherwise manage",
        "additive updates only",
        "codex execpolicy check",
    ]:
        assert needle in text


def test_cxpp_status_is_read_only_and_checks_host_pointers() -> None:
    text = read_skill("cxpp-status")

    assert "skill is read-only" in text
    assert "codex plugin list --available" in text
    assert "`installed` or `missing`" in text
    assert "codex mcp get second-opinion" in text
    assert "curl -sf http://127.0.0.1:8080/readyz" in text


def test_cxpp_status_inventory_matches_every_marketplace_family() -> None:
    marketplace = json.loads(MARKETPLACE.read_text(encoding="utf-8"))
    marketplace_families = tuple(plugin["name"] for plugin in marketplace["plugins"])
    text = read_skill("cxpp-status")

    assert marketplace_families == FULL_FAMILIES
    for family in FULL_FAMILIES:
        assert f"`{family}`" in text


def test_cxpp_init_and_update_offer_idempotent_suite_profiles() -> None:
    for name in ("cxpp-init", "cxpp-update"):
        text = read_skill(name)

        for profile in ("Minimal", "Recommended", "Full suite", "Custom"):
            assert f"**{profile}**" in text
        for family in RECOMMENDED_FAMILIES:
            assert f"`{family}`" in text
        for field in ("previous ref", "requested", "resolved SHA", ".agents", "plugins/<family>"):
            assert field in text
        for outcome in ("updated", "already current", "skipped by user", "needs host prerequisite"):
            assert outcome in text
        assert "floating" in text
        assert "idempotent" in text or "Re-running" in text


def test_suite_consent_does_not_authorize_host_changes() -> None:
    for name in ("cxpp-init", "cxpp-update"):
        text = read_skill(name)

        for boundary in ("MCP pointers", "credentials", "hooks", "exec-policy rules", "external-service"):
            assert boundary in text


def test_cxpp_plugin_includes_the_host_pointer_template() -> None:
    assert (PLUGIN / "config.toml.example").read_bytes() == (ROOT / "templates" / "config.toml.example").read_bytes()


def test_cxpp_update_retains_reviewed_hook_roots_across_transitions() -> None:
    text = read_skill("cxpp-update")

    for required in (
        "cxpp-hook-transition.py",
        "preflight",
        "reinstall",
        "byte-identical",
        "separate hook-family installs",
        "reused version path whose bytes changed",
        "recover --marketplace codex-power-pack --approve",
        "external terminal",
        "new Codex session plus `/hooks`",
    ):
        assert required in text
