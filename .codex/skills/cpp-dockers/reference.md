...
upstream @playwright/mcp npx/stdio server (no container; see $cxpp-init).
```
...
- **Browser automation missing:** register the upstream server with `$cxpp-init`, or `claude mcp add --transport stdio --scope user playwright -- npx -y @playwright/mcp@latest --headless`
