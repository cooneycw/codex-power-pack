...
Codex skills. claude-power-pack (CPP) authors the shared command families once in
...
Reconcile rule (hybrid SoT): edit the SOURCE in claude-power-pack
`.claude/commands/<family>/`, regenerate CPP's `codex/skills/` (`make codex-skills`
there), then re-run this with `--refresh` here. Never hand-edit `.codex/skills/`.
"""
...
PULL_EXCLUDE_FAMILIES = {"claude-md"}

# CxPP-owned files that live under .codex/skills/ but are NOT pulled from CPP, so the
# drift manifest and the copy/prune logic must leave them alone.
...
def _generated_files() -> list[Path]:
    ...
    Top-level files (the CxPP-authored README.md) are excluded: they are owned here,
    not vendored, so they are not part of the drift surface.
    """
    ...
    return sorted(
        ...
        if p.is_file() and p.relative_to(SKILLS_ROOT).parts[0] not in LOCAL_FILES
...
def run_check() -> int:
    ...
    print(f"codex-skills-sync: {len(actual)} file(s) across {len(_skill_dirs())} skill(s) in sync")
    return 0
...
def run_refresh(cpp_root: Path, ref: str) -> int:
    ...
    # Prune generated skill dirs that no longer exist upstream (keep CxPP-owned files).
