...
def _payload_set_digest(payloads: dict[str, bytes]) -> str:
    """Digest a path-addressed payload set without concatenation ambiguity."""
    entries = [
...
def _validate_reporting_baseline() -> tuple[Pin, str, str, dict[str, bytes], str, str]:
    ...
    plugin_drift, _ = _plugin_payload_drift(actual)
    drift.extend(plugin_drift)
    if drift:
        raise IntegrityError(f"report baseline has {len(drift)} integrity mismatch(es)")
    for rel in sorted(actual):
        committed = _git_bytes(REPO_ROOT, "show", f"{head}:{PIN_PULL_DESTINATION}{rel}")
...
def _upstream_report(
    ...
    *,
    ...
    pin, cxpp_head, cxpp_tree, actual, overlay_digest, manifest_digest = (
    ...
    _, by_skill, expected = _prepare_source_payloads(cpp_root, commit=ref)
    ...
    return {
        ...
        "baseline": {
            ...
            "adapted_payload_sha256": _payload_set_digest(actual),
            "files": len(actual),
        },
        "target": {
            "repo": CPP_REPO_URL,
            ...
            "adapted_payload_sha256": _payload_set_digest(expected),
            "files": len(expected),
        },
        "overlay": {
